#pragma once
#include "cedilla/generated_props_casing.hpp"
#include <span>
#include <ranges>
...
namespace details {
    ...
    constexpr inline casing_data casing_datas [] = {
        {uni::detail::casing_uc_data, uni::detail::casing_uc_starts, uni::detail::casing_uc_length},
        {uni::detail::casing_lc_data, uni::detail::casing_lc_starts, uni::detail::casing_lc_length},
        {uni::detail::casing_tc_data, uni::detail::casing_tc_starts, uni::detail::casing_tc_length},
        {uni::detail::casing_cf_data, uni::detail::casing_cf_starts, uni::detail::casing_cf_length}
    };
    ...
    constexpr auto get_case_maping(char32_t c) {
        ...
        for (std::size_t i = 0; i < d.length; i++) {
            ...
            auto it = std::lower_bound(start, end, c, [](const char32_t(&mapping)[2], const char32_t probe) {
    ...
    class casing_view : public std::ranges::view_interface<casing_view<V, CTK>>{
        V base_;
    public:
        class iterator {
        ...
        public:
            ...
            constexpr iterator(const casing_view* p, BaseIt it) noexcept
                : parent(p), base_(it) {
                if(base_ != std::ranges::end(parent->base())) {
                    b_ = get_case_maping<CTK>(*base_);
                }
            ...
            constexpr bool operator==(const iterator& other) const requires std::equality_comparable<BaseIt> {
    ...
    struct casing_view_fn {
        ...
        constexpr auto operator()(R &&r) const {
            return casing_view<std::views::all_t<R>, CTK>{std::forward<R>(r)};
        }

        template <std::ranges::input_range R>
        constexpr friend auto operator|(R &&rng, const casing_view_fn &) {
            return casing_view<std::views::all_t<R>, CTK>{std::forward<R>(rng)};
        }
    };
}
...
namespace views {
    inline details::casing_view_fn<details::case_transformation_upper> uppercase;
    inline details::casing_view_fn<details::case_transformation_lower> lowercase;
    inline details::casing_view_fn<details::case_transformation_fold>  casefold;
}
